...
def welcome_handler(state: State, message: Message) -> Tuple[State, Message]:
    """Initialize a greeting and explain the service."""
    ...
    if not state.get("initialized"):
        ...
        state["conversation"] = {"history": [], "current_flow": "welcome"}
...
def legal_handler(state: State, message: Message) -> Tuple[State, Message]:
    ...
    if (
        state["conversation"]["current_flow"] == "legal"
        and state["lead_data"]["consentimiento"] is None
    ):
        ...
        if consent_given:
            ...
            response = "Gracias por tu consentimiento. Ahora cuéntame, ¿cómo te llamas?"
...
def core_data_handler(state: State, message: Message) -> Tuple[State, Message]:
    ...
    if state["lead_data"]["nombre"] is None:
        ...
        if state["lead_data"]["tipo_inmueble"]:
            inmueble = state["lead_data"]["tipo_inmueble"]
            response = (
                f"Gracias, {name}. Para mostrarte opciones de {inmueble} "
        ...
        if state["lead_data"]["tipo_inmueble"] is None:
            ...
            nombre = state["lead_data"]["nombre"]
            response = (
                f"Entendido, {nombre}. Para mostrarte opciones de {property_type} "
                f"que podrían interesarte, ¿me podrías compartir tu número de celular?"
            )
            state["conversation"]["current_flow"] = "contact"
        else:
...
def build_mvp_graph():
    ...
    graph.add_conditional_edges(
        "legal",
        lambda state: (
            "core_data" if state["lead_data"]["consentimiento"] is True else "legal"
        ),
    )
