...
import pytest
from src.utils.extractors import extract_property_type, extract_consent, extract_phone_number, extract_email
...
def test_extract_email():
    ...
    assert extract_email("Contacto: usuario.nombre@dominio.com") == "usuario.nombre@dominio.com"
    assert extract_email("Email de trabajo usuario-nombre+etiqueta@dominio.com") == "usuario-nombre+etiqueta@dominio.com"
    
    # Test domain variations
    assert extract_email("Usar empresa@dominio.co.pe para contacto") == "empresa@dominio.co.pe"
    assert extract_email("En mi-empresa@sub.dominio.net están los detalles") == "mi-empresa@sub.dominio.net"
    
    # Test within a sentence
    assert extract_email("Para más información escríbeme a info@empresa.com y te responderé pronto") == "info@empresa.com"
    
    # Test multiple emails (should return the first one)
    assert extract_email("Puedes usar admin@sistema.com o soporte@sistema.com") == "admin@sistema.com"
